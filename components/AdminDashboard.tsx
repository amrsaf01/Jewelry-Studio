...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({ config, onUpdate, onClose }) => {
    ...
    const handleLogoUpload = async (e: React.ChangeEvent<HTMLInputElement>, target: 'app' | 'watermark') => {
        ...
        try {
            ...
            if (target === 'app') {
                ...
                updateWatermark({ logoUrl: data.publicUrl });
            }
        } catch (error) {
                ...
                <div className="w-64 bg-stone-50 border-r border-stone-200 p-6 flex flex-col">
                    ...
                    <div className="mt-auto pt-6 border-t border-stone-200 space-y-4">
                        <button
                            ...
                            className="flex items-center gap-2 text-xs text-red-500 hover:text-red-700 font-bold px-4 transition-colors"
                        >
                            <Trash2 size={14} /> Reset App Data
                        </button>
                        <p className="text-xs text-stone-400 px-4">CMS Version 1.2-DEBUG</p>
                    </div>
                </div>
